...
occs_Pa <- occs %>% 
  select("key","scientificName.x", "latitude.x", "longitude.x", "speciesKey.x" , "scientificName.new.TPL.y") %>% 
  rename(Name = scientificName.x,lat = latitude.x, long = longitude.x, Name_new = scientificName.new.TPL.y) %>% 
  filter(Name_new == "Pinus armandii")

# 保存预下载的物种数据
fwrite(occs_Pa,
       "datas/Occs/Pinus_armandii.csv", 
       na = "NA")
```


```{r 去重}
# 下载的原始数据，几乎总是会有一些错误点。例如同一地点的物种记录出现重复，删除具有相同坐标的记录点是一个很好的做法，以避免假性复制。 
occs_dups <- duplicated(occs_Pa[,c('long', 'lat')])
occs_Pa <- occs_Pa[!occs_dups,]
# 确保经度和纬度的数据类型为 “数字型” (sometimes they are characters)
occs_Pa$lat <- as.numeric(occs_Pa$lat)
occs_Pa$long <- as.numeric(occs_Pa$long)


# 转化为sf矢量对象，便于地图加载, 指定crs
occs_sf <- st_as_sf(occs_Pa, coords = c("long", "lat"), crs = 4326)
occs_sf <- bind_cols(occs_sf, as.data.frame(st_coordinates(occs_sf)))
# 可以选择某个矩形范围内，然后将其之外的剔除
# 使用sf 包 
qtp <- st_read("datas/StudyArea/TPboundaryPolygon.shp")

# 两图层crs要一致
int <-st_transform(qtp, crs(occs_sf))
# 在地图上显示
...
```{r}
# 将记录所在行号作为其 unique ID，并添加列
occs_sf$occID <- row.names(occs_sf)
occs_sf <- occs_sf %>% filter(!(occs_sf$occID %in% c(43)))
```
...
thinnned_dataset_full <- spThin::thin(loc.data = occs_sf,
...
View(thinnned_dataset_full[[1]])

# subset occs to match only thinned occs
occs_sf <- occs_sf[as.numeric(rownames(thinnned_dataset_full[[1]])),]
...
# 保存预处理后的物种数据
fwrite(occs_sf,
       "datas/Occs/Pinus_armandii_processed.csv", 
       na = "NA")
```
...
# 掩膜提取int范围的bio
bio_int <- terra::crop(bio, int)
bio_int <- terra::mask(bio_int, int)

# 本地加载地形变量(terrains)
topo_file <- list.files("datas/Terrains/", pattern = ".tif$", full.names = T, recursive = T)
topo <- topo_file %>% rast()
names(topo) <- c('aspect', 'elev', 'slope')

# 统一topo的投影为：WGS_84坐标系
topo <- project(topo, bio_int, method = "bilinear")

writeRaster(c(bio_int, topo), "datas/current_bio/envs_qtp_22_4326.tif", overwrite=TRUE)
envs <- rast("datas/current_bio/envs_qtp_22_4326.tif")
```
...
# 选定背景区域范围
occs_buf <- st_buffer(occs_sf, dist = 300000) %>% st_union() %>% st_sf()
plot(occs_buf)

# crop the environmental rasters by the background extent shape
envs_bg <- terra::crop(envs, vect(occs_buf))
# mask the background extent shape from the cropped raster
envs_bg <- terra::mask(envs_bg, vect(occs_buf))

plot(envs_bg["bio01"]/10)
```
...
bg <- fread("Target_group_background/Pa_bg_points.csv") %>% 
  select('POINT_X', 'POINT_Y')
bg <- dismo::randomPoints(raster::brick(envs_bg), n = 10000) %>% as.data.table()
colnames(bg) <- c('X', 'Y')
...
e <- ENMeval::ENMevaluate(st_drop_geometry(occs_sf)[, c('X', 'Y')], 
                 envs_bg, 
                 bg, 
                 tune.args = tune.args, 
                 partitions = "block", 
...
occs.z <- cbind(occs, terra::extract(envs_bg, vect(occs_sf))) %>% dplyr::select(-c(ID, name, occID))
bg.z <- cbind(bg, terra::extract(envs_bg, bg)) %>% dplyr::select(-ID)
...
evalplot.stats(e = e, stats = c("or.10p", "auc.val", "cbi.val"), color = "fc", x.var = "rm", error.bars = T, dodge = 0.1)
```

```{r}
# 最后，我们可以切换哪些变量在X轴上，哪些变量用颜色来象征。
# ENMeval目前一次只接受两个变量用于绘图。
evalplot.stats(e = e, stats = c("or.10p", "auc.val", "cbi.val"), color = "rm", x.var = "fc", 
               error.bars = FALSE)
```
...
# 我们还可以将分档后的背景点与发生点绘制在上面，以便可视化训练数据的位置。
...
mod.null <- ENMnulls(e, mod.settings = list(fc = "LQ", rm = 2), no.iter = 100)
...
```{r}
projPoly <- st_read("datas/StudyArea/Qinghai_Province.json")

predsProj_seq <- terra::subset(envs_int, names(envs_bg_seq))
...
writeRaster(proj_Current$xferArea, "result/Maps/Pa_proj_Current.tif")

Current <- terra::project(rast(proj_now), elev, method = "bilinear")
writeRaster(Current, "result/Maps/Current_tune.tif")
plot(proj_now, main = "现今QTP分布范围")
```

## 全新世中期(6 ka)投射
```{r}
mid_file <- list.files("datas/Paleo_bio/ccmidbi_30s", pattern = ".tif$", full.names = T, recursive = T)
bio_mid <- MID_file %>% rast()
names(bio_mid) <- paste0('bio', sprintf("%02d", 1:19))

bio_mid_int <- terra::crop(bio_mid, int)
bio_mid_int <- terra::mask(bio_mid_int, int)

writeRaster(c(bio_mid_int, topo), "datas/Paleo_bio/mid_envs_qtp_22_4326.tif", overwrite=TRUE)
envs_mid <- rast("datas/Paleo_bio/mid_envs_qtp_22_4326.tif")
...
rangeModelMetadata::rmmToCSV(rmm, "result/log_text/Pa_rmm_0820.csv")
```