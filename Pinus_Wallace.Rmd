...
```{r 加载occs}
# 提取物种、经度、纬度列
occs <- fread("datas/Occs/Pinus_data_coordinate_check.csv") %>%
...
Occs <- occs_userOccs(
  txtPath = "datas/Occs/Pinus_data_coordinate_check_wallace.csv",
  txtName = "Pinus_data_coordinate_check_wallace.csv",
  txtSep = ",",
  txtDec = "."
)
...
```{r 在线envs}
# geodata::worldclim_global(var = "bio", res = 10, path="datas/current_bio")
# 新文件重命名
# bio_file <- list.files("datas/current_bio/wc2.1_10m/", pattern = ".tif$", full.names = T, recursive = T)
# for (i in 1:length(bio_file)){
#   newname <- paste0("datas/current_bio/wc2.1_10m/",
#                     "wc2.1_10m_",
#                     c("bio01", 
#                       paste0("bio", 10:19), 
...
bio_file <- list.files("datas/current_bio/Chelsa_30s/RasterEnvs/", pattern = ".tif$", full.names = T, recursive = T)
...
### Pa_Pd (*Pinus armandii* and *Pinus densata*)
...
# 选择分析的环境变量
pcaSel_Pa_Pd <- c("bio01", "bio02", "bio03", "bio04", "bio05", "bio06", "bio07", "bio08", "bio09", "bio10", "bio11", "bio12", "bio13", "bio14", "bio15", "bio16", "bio17", "bio18", "bio19")

# Run the pca
espace_pca_Pa_Pd <- espace_pca(
  sp.name1 = "Pinus armandii",
  sp.name2 = "Pinus densata",
  occs.z1 = occs_Pa[, pcaSel_Pa_Pd],
  occs.z2 = occs_Pd[, pcaSel_Pa_Pd],
  bgPts.z1 = bgEnvsVals_Pa[, pcaSel_Pa_Pd],
  bgPts.z2 = bgEnvsVals_Pd[, pcaSel_Pa_Pd]
)
```
...
# PCA Scatter Plot
if ("occsBg" == "occs") {
  x <- espace_pca_Pa_Pd$scores[espace_pca_Pa_Pd$scores$bg == "sp", ]
  x.f <- factor(x$sp)
} else if ("occsBg" == "occsBg") {
  x <- espace_pca_Pa_Pd$scores[espace_pca_Pa_Pd$scores$sp == "bg", ]
  x.f <- factor(x$bg)
}
ade4::s.class(x, x.f,
  xax = 1, yax = 2,
...
ade4::s.corcircle(espace_pca_Pa_Pd$co,
  xax = 1, yax = 2,
  lab = pcaSel_Pa_Pd, full = FALSE, box = TRUE
)

# PCA screeplot
screeplot(espace_pca_Pa_Pd, main = NULL)
# Print PCA summary of results
summary(espace_pca_Pa_Pd)
```
...
```{r Pa_Pd Dens}
# Create density grid
espace_occDens_Pa_Pd <- espace_occDens(
  sp.name1 = "Pinus armandii",
  sp.name2 = "Pinus densata",
  pca = espace_pca_Pa_Pd
)
# Plots
graphics::par(mfrow = c(1, 2))
ecospat::ecospat.plot.niche(espace_occDens_Pa_Pd[["Pinus armandii"]],
  title = "Pinus armandii"
)
ecospat::ecospat.plot.niche(espace_occDens_Pa_Pd[["Pinus densata"]],
  title = "Pinus densata"
)
```
...
```{r Pa_Pd nicheOv}
## Run tests
espace_nicheOv_Pa_Pd <- espace_nicheOv(
  z1 = espace_occDens_Pa_Pd[["Pinus armandii"]],
  z2 = espace_occDens_Pa_Pd[["Pinus densata"]],
  iter = 100,
  similarity = TRUE
)
...
ecospat::ecospat.plot.niche.dyn(
  espace_occDens_Pa_Pd[["Pinus armandii"]],
  espace_occDens_Pa_Pd[["Pinus densata"]],
  0.5,
  title = "Pinus armandii_Pinus densata",
  col.unf = "blue",
  col.exp = "red",
  col.stab = "purple",
  colZ1 = "blue",
...
ecospat::ecospat.plot.overlap.test(
  espace_nicheOv_Pa_Pd$simil,
  "D", "Similarity test"
)
```

### Pa_Py (*Pinus armandii* and *Pinus yunnanensis*)
...
```{r Pa_Py PCA}
# 对*Pinus armandii*和*Pinus yunnanensis*的环境空间进行PCA分析并绘图，以降低维度。
# 选择对occsBg进行PCA分析。

# 选择分析的环境变量
pcaSel_Pa_Py <- c("bio01", "bio02", "bio03", "bio04", "bio05", "bio06", "bio07", "bio08", "bio09", "bio10", "bio11", "bio12", "bio13", "bio14", "bio15", "bio16", "bio17", "bio18", "bio19")

# Run the pca
espace_pca_Pa_Py <- espace_pca(
  sp.name1 = "Pinus armandii",
  sp.name2 = "Pinus yunnanensis",
  occs.z1 = occs_Pa[, pcaSel_Pa_Py],
  occs.z2 = occs_Py[, pcaSel_Pa_Py],
  bgPts.z1 = bgEnvsVals_Pa[, pcaSel_Pa_Py],
  bgPts.z2 = bgEnvsVals_Py[, pcaSel_Pa_Py]
)
```
...
# PCA Scatter Plot
if ("occsBg" == "occs") {
  x <- espace_pca_Pa_Py$scores[espace_pca_Pa_Py$scores$bg == "sp", ]
  x.f <- factor(x$sp)
} else if ("occsBg" == "occsBg") {
  x <- espace_pca_Pa_Py$scores[espace_pca_Pa_Py$scores$sp == "bg", ]
  x.f <- factor(x$bg)
}
ade4::s.class(x, x.f,
  xax = 1, yax = 2,
...
# PCA Correlation circle
ade4::s.corcircle(espace_pca_Pa_Py$co,
  xax = 1, yax = 2,
  lab = pcaSel_Pa_Py, full = FALSE, box = TRUE
)

# PCA screeplot
screeplot(espace_pca_Pa_Py, main = NULL)
# Print PCA summary of results
summary(espace_pca_Pa_Py)
```

#### Occs密度分布 (Occurrence Density Grid)

计算*Pinus armandii*和*Pinus yunnanensis*的环境空间中物种密集分布的部分和背景环境条件的可用性

```{r Pa_Py Dens}
# Create density grid
espace_occDens_Pa_Py <- espace_occDens(
  sp.name1 = "Pinus armandii",
  sp.name2 = "Pinus yunnanensis",
  pca = espace_pca_Pa_Py
)
# Plots
graphics::par(mfrow = c(1, 2))
ecospat::ecospat.plot.niche(espace_occDens_Pa_Py[["Pinus armandii"]],
  title = "Pinus armandii"
)
ecospat::ecospat.plot.niche(espace_occDens_Pa_Py[["Pinus yunnanensis"]],
  title = "Pinus yunnanensis"
)
```

#### 生态位重叠度 (Niche overlap)

评估*Pinus armandii*与*Pinus yunnanensis*之间的生态位重叠度，并计算出其occurrence density grid。运行equivalence test (FALSE)和similarity test TRUE.

生态位重叠度是基于模块Occurrence Density Grid中估计的可用环境空间中的Occs密度和bg密度。重叠度是用Schoener's D指标进行量化的。

```{r Pa_Py nicheOv}
## Run tests
espace_nicheOv_Pa_Py <- espace_nicheOv(
  z1 = espace_occDens_Pa_Py[["Pinus armandii"]],
  z2 = espace_occDens_Pa_Py[["Pinus yunnanensis"]],
  iter = 100,
  similarity = TRUE
)
...
# graphics::par(mfrow = c(1,2))
ecospat::ecospat.plot.niche.dyn(
  espace_occDens_Pa_Py[["Pinus armandii"]],
  espace_occDens_Pa_Py[["Pinus yunnanensis"]],
  0.5,
  title = "Pinus armandii_Pinus yunnanensis",
  col.unf = "blue",
  col.exp = "red",
  col.stab = "purple",
  colZ1 = "blue",
...
# Plot
ecospat::ecospat.plot.overlap.test(
  espace_nicheOv_Pa_Py$simil,
  "D", "Similarity test"
)
```

### Pd_Py (*Pinus densata* and *Pinus yunnanensis*)

#### 环境排序分析(Environmental Ordination analysis)

环境排序是为了进行主成分分析（PCA），它旋转多维数据（例如，对于许多变量），以描述显示最高变化的轴的子集。要进行PCA，通过勾选/不勾选生物气候变量，选择两个物种可用的变量。选择 "仅发生率"或 "发生率和背景"作为图的选择，并设置X轴和Y轴的组成部分。PCA散点图出现在结果标签中。[\@Wallace](https://wallaceecomod.github.io/wallace/articles/tutorial-v2.html)

```{r Pd_Py PCA}
# 对*Pinus densata*和*Pinus yunnanensis*的环境空间进行PCA分析并绘图，以降低维度。
# 选择对occsBg进行PCA分析。

# 选择分析的环境变量
pcaSel_Pd_Py <- c("bio01", "bio02", "bio03", "bio04", "bio05", "bio06", "bio07", "bio08", "bio09", "bio10", "bio11", "bio12", "bio13", "bio14", "bio15", "bio16", "bio17", "bio18", "bio19")

# Run the pca
espace_pca_Pd_Py <- espace_pca(
  sp.name1 = "Pinus densata",
  sp.name2 = "Pinus yunnanensis",
  occs.z1 = occs_Pd[, pcaSel_Pd_Py],
  occs.z2 = occs_Py[, pcaSel_Pd_Py],
  bgPts.z1 = bgEnvsVals_Pd[, pcaSel_Pd_Py],
  bgPts.z2 = bgEnvsVals_Py[, pcaSel_Pd_Py]
)
```
...
# PCA Scatter Plot
if ("occsBg" == "occs") {
  x <- espace_pca_Pd_Py$scores[espace_pca_Pd_Py$scores$bg == "sp", ]
  x.f <- factor(x$sp)
} else if ("occsBg" == "occsBg") {
  x <- espace_pca_Pd_Py$scores[espace_pca_Pd_Py$scores$sp == "bg", ]
  x.f <- factor(x$bg)
}
ade4::s.class(x, x.f,
  xax = 1, yax = 2,
...
# PCA Correlation circle
ade4::s.corcircle(espace_pca_Pd_Py$co,
  xax = 1, yax = 2,
  lab = pcaSel_Pd_Py, full = FALSE, box = TRUE
)

# PCA screeplot
screeplot(espace_pca_Pd_Py, main = NULL)
# Print PCA summary of results
summary(espace_pca_Pd_Py)
```
...
```{r Pd_Py Dens}
# Create density grid
espace_occDens_Pd_Py <- espace_occDens(
  sp.name1 = "Pinus densata",
  sp.name2 = "Pinus yunnanensis",
  pca = esPdce_pca_Pd_Py
)
# Plots
graphics::par(mfrow = c(1, 2))
ecospat::ecospat.plot.niche(esPdce_occDens_Pd_Py[["Pinus densata"]],
  title = "Pinus densata"
)
ecospat::ecospat.plot.niche(esPdce_occDens_Pd_Py[["Pinus yunnanensis"]],
  title = "Pinus yunnanensis"
)
```
...
评估*Pinus densata*与*Pinus yunnanensis*之间的生态位重叠度，并计算出其occurrence density grid。运行equivalence test (FALSE)和similarity test TRUE.

生态位重叠度是基于模块Occurrence Density Grid中估计的可用环境空间中的Occs密度和bg密度。重叠度是用Schoener's D指标进行量化的。

```{r Pd_Py nicheOv}
## Run tests
espace_nicheOv_Pd_Py <- espace_nicheOv(
  z1 = espace_occDens_Pd_Py[["Pinus densata"]],
  z2 = espace_occDens_Pd_Py[["Pinus yunnanensis"]],
  iter = 100,
  similarity = TRUE
)
...
# graphics::par(mfrow = c(1,2))
ecospat::ecospat.plot.niche.dyn(
  espace_occDens_Pd_Py[["Pinus densata"]],
  espace_occDens_Pd_Py[["Pinus yunnanensis"]],
  0.5,
  title = "Pinus armandii_Pinus yunnanensis",
  col.unf = "blue",
  col.exp = "red",
